...
ASEGenes <- tibble(expressedGene = random_gene[1:numASE]@ranges@NAMES) %>%
  separate(col = expressedGene, sep=" ", 
           into = c("transcript", 'length'), fill = "right") 
...
write.table(expressGeneTibble, file = paste0(out, "/expressedGene.tsv"), row.names = F, col.names = T, quote = F)
